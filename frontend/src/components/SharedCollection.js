...
const CamisetaItem = React.memo(({ camiseta, camisetas, currentIndex, onNavigate }) => {
  ...
  return (
    <>
      <div className="camiseta-item" onClick={() => setShowModal(true)}>
          ...
          <div className="detalle-container" onClick={(e) => e.stopPropagation()}>
            <div className="detalle-header">
              <div className="header-buttons">
                ...
                {currentIndex > 0 && (
                  <button className="btn-nav btn-prev" onClick={() => onNavigate(currentIndex - 1)}>
                ...
                {currentIndex < camisetas.length - 1 && (
                  <button className="btn-nav btn-next" onClick={() => onNavigate(currentIndex + 1)}>
    ...
    const fetchCollectionData = async () => {
      try {
        ...
        const camisetasResponse = await fetch(
          `http://localhost:8080/api/shared/camisetas/${token}`,
        );
  ...
  const applyFilters = () => {
    ...
    Object.entries(activeFilters).forEach(([key, value]) => {
      if (Array.isArray(value) && value.length > 0) {
        ...
        filtered = filtered.filter((camiseta) => (value ? camiseta.dorsal : !camiseta.dorsal));
      }
    });
              ...
              <div className="user-info-left">
                <div className="user-photo-section">
                  {collectionData.userPhoto ? (
                    <img src={collectionData.userPhoto} alt="User" className="user-photo" />
                  ) : (
                    <div className="user-photo-placeholder">
                      {collectionData.username.charAt(0).toUpperCase()}
                    </div>
                  )}
                  <span className="collection-title">Colección de {collectionData.username}</span>
                </div>
