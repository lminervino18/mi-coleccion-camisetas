...
const serverEnvSchema = z.object({
  ...
  /**
   * Optional on purpose: a deployment without a database still builds and serves the pages that
   * do not read data, instead of failing the whole build. Anything that needs persistence
   * reports it at request time.
   */
  DATABASE_URL: z.string().url('DATABASE_URL debe ser una URL de conexión válida.').optional(),
  APP_URL: z
    .string()
